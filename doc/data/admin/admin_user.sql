...
(
    ...
    `user_no`             char(24)    NOT NULL COMMENT '用户编号 AUNO+yyyymmddhhmmss+1位随机大写字母+5位随机数字',
    `user_name`           varchar(50) NOT NULL COMMENT '用户名称',
    `user_account`        varchar(50) NOT NULL COMMENT '用户帐号',
    `user_password`       varchar(32) NOT NULL COMMENT '用户密码',
    `role_no`             char(24)    NOT NULL COMMENT '用户角色',
    ...
    PRIMARY KEY (`user_id`),
    UNIQUE INDEX `idx_admin_user_no` (`user_no`),
    UNIQUE INDEX `idx_admin_user_account` (`user_account`)
) ENGINE = InnoDB
  CHARACTER SET = utf8;