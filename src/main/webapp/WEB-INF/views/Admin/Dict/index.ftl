...
<div class="x-nav">
    ...
    <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right"
       onclick="window.location.href='/admin/dict/view.do'" title="刷新">
        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-body ">
                    <form class="layui-form layui-col-space5" lay-filter="search-box">
                        <div class="layui-inline layui-show-xs-block">
                            <input type="text" name="dictName" placeholder="请输入字典名称" autocomplete="off"
                                   class="layui-input" id="dictName">
                        </div>
                        <div class="layui-inline layui-show-xs-block">
                ...
                <div class="layui-card-header">
                    <@shiro.hasPermission name="adminDict:deleteAll">
                        <button class="layui-btn layui-btn-danger" onclick="delAll()">
                            <i class="layui-icon"></i>批量删除
                        </button>
                    </@shiro.hasPermission>
                    <@shiro.hasPermission name="adminDict:add">
                        <button class="layui-btn" onclick="xadmin.open('添加用户','/admin/dict/add_page.do',600,500)">
                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form">
                        ...
                        <#list dictList as dict>
                            <tr>
                                <td>
                                    <input type="checkbox" name="id" value="${dict.dictNo}" lay-skin="primary">
                                </td>
                                <td>${dict_index + 1}</td>
                                <td>${dict.dictName}</td>
                                <td>${dict.dictKey}</td>
                                <td>${dict.dictValue}</td>
                                <td>${dict.dictIndex}</td>
                                <td>${dict.menuName}</td>
                                <td class="td-manage">
                                    <@shiro.hasPermission name="adminDict:edit">
                                        <a title="编辑" onclick="xadmin.open('编辑','/admin/dict/edit_page.do?dictNo=${dict.dictNo}',600,500)" href="javascript:;">
                                            <i class="layui-icon">&#xe642;</i>
                                        </a>
                                    </@shiro.hasPermission>
                                    <@shiro.hasPermission name="adminDict:delete">
                                        <a title="删除" onclick="deleteDict('${dict.dictNo}')" href="javascript:;">
                                            <i class="layui-icon">&#xe640;</i>
                                        </a>
                                    </@shiro.hasPermission>
                                </td>
...
<script>
    ...
    function delAll(argument) {
        ...
        layer.confirm('确认要删除吗？', function (index) {
            $.ajax({
                url: "/admin/dict/delete_batch.do",
                type: 'post',
                data: {dictNos: ids.toString()},
    ...
    // 删除用户
    function deleteDict(dictNo) {
        layer.confirm('确认要删除吗？', function (index) {
            //发异步删除数据
            $.ajax({
                url: "/admin/dict/delete.do",
                type: 'post',
                data: {dictNo: dictNo},
                success: function (res) {
                    var icon = 5;
    ...
    function queryPage(currentPage) {
        ...
        window.location.href = "/admin/dict/view.do" + "?conditions=" + encodeURI(JSON.stringify(data)) + "&currentPage=" + currentPage;
    }
